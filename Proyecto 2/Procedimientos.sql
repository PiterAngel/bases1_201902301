...
CrearCarrera:BEGIN
	IF (validar_letras(p_nombre) = 0) THEN
        SELECT 'Error: solo puede contener letras' as ERROR;
        LEAVE CrearCarrera;
    END IF;
    IF (ExisteCarreraPorNombre(p_nombre) = 1) THEN
        SELECT 'Error: La carrera ya existe' as ERROR;
        LEAVE CrearCarrera;
    END IF;
    INSERT INTO Carrera (Nombre)
    VALUES (p_nombre);
    SELECT 'Éxito: Carrera registrada' as MENSAJE;
END $$
...
DECLARE telefono_final INT;
    ...
    IF (ExisteEstudiante(Carnet) = 1) THEN
        SELECT 'Error: El estudiante ya existe' as ERROR;
        LEAVE RegistrarEstudiante;
    END IF;
    IF (validar_letras(Nombre) = 0) THEN
        SELECT 'Error: El nombre solo debe contener letras' as ERROR;
        LEAVE RegistrarEstudiante;
    END IF;
    IF (validar_letras(Apellido) = 0) THEN
        SELECT 'Error: El apellido solo debe contener letras' as ERROR;
        LEAVE RegistrarEstudiante;
    END IF;
    IF (validar_correo(Correo) = 0) THEN
        SELECT 'Error: Correo con formato inválido' as ERROR;
        LEAVE RegistrarEstudiante;
    END IF;
    IF (ExisteCarreraPorId(idCarrera) = 0) THEN
        SELECT 'Error: La carrera no existe' as ERROR;
        LEAVE RegistrarEstudiante;
    END IF;
    IF (ExistePlanPorId(idPlan) = 0) THEN
        SELECT 'Error: El plan no existe' as ERROR;
        LEAVE RegistrarEstudiante;
    END IF;
    IF (CoindidePlanCarrera(IdCarrera, IdPlan) = 0) THEN
        SELECT 'Error: El plan no coincide con la carrera' as ERROR;
        LEAVE RegistrarEstudiante;
    END IF;
    ...
    SELECT 'Éxito: Estudiante registrado' as MENSAJE;
END $$
...
DECLARE telefono_final INT;
    ...
    IF (ExisteCatPorCodigo(CodigoCat) = 1) THEN
		SELECT 'Error: El catedratico ya existe' as ERROR;
    ...
    IF (validar_enteroPositivo(salario) = 0) THEN
        SELECT 'Error: El salario debe ser un número positivo' as ERROR;
        LEAVE RegistrarDocente;
    END IF;
    IF (salario > 99000) THEN
        SELECT 'Error: El salario no puede ser mayor a 99000' as ERROR;
        LEAVE RegistrarDocente;
    END IF;
    -- Validar y convertir el teléfono
    SET telefono_final = validar_telefono(Telefono);
    ...
    SELECT 'Éxito: Catedratico registrado' as MENSAJE;
END $$
...
CrearCurso:BEGIN
    ...
    IF (ExistePensum(CodigoCurso) = 1) THEN
        SELECT 'Error: Curso ya existente en el pensum' as ERROR;
        LEAVE CrearCurso;
    END IF;
    IF (validar_enteroPositivo(Creditos_necesarios) = 0) THEN
        SELECT 'Error: Los creditos necesarios deben ser un número positivo' as ERROR;
        LEAVE CrearCurso;
    END IF;
    IF (validar_enteroPositivo(Creditos_otorga) = 0) THEN
        SELECT 'Error: Los creditos que otorga debe ser un número positivo' as ERROR;
        LEAVE CrearCurso;
    END IF;
    IF (Obligatorio != 0 and Obligatorio != 1) THEN
        SELECT 'Error: Debe ser 1 o 0' as ERROR;
        LEAVE CrearCurso;
    END IF;
    IF (ExistePlanPorId(idPlan) = 0) THEN
        SELECT 'Error: El plan no existe' as ERROR;
    ...
    SELECT 'Éxito: Curso creado' as MENSAJE;
END $$
...
-- --------------------------------------------------------
DROP PROCEDURE IF EXISTS AgregarPrerrequisito;
DELIMITER $$
...
AgregarPrerrequisito:BEGIN
    ...
    IF (ExisteCurso(Curso) = 0) THEN
        SELECT 'Error: El curso no existe' as ERROR;
        LEAVE AgregarPrerrequisito;
    END IF;
    IF (ExisteCurso(prerreq) = 0) THEN
        SELECT 'Error: El Prerrequisito no existe' as ERROR;
        LEAVE AgregarPrerrequisito;
    END IF;

    INSERT INTO Prerrequisito (Curso, prerrequisito)
    VALUES (Curso, prerreq);
    SELECT 'Éxito: Prerrequisito creado' as MENSAJE;
END $$
...
CrearSeccionCurso:BEGIN
    ...
    IF (CSDuplicados(CodigoCurso, Seccion) = 1) THEN
        SELECT 'Error: Sección repetida para un curso en el mismo ciclo' as ERROR;
        LEAVE CrearSeccionCurso;
    END IF;
    IF (ValidarCiclo(Ciclo) = 0) THEN
        SELECT 'Error: Ciclo invalido' as ERROR;
        LEAVE CrearSeccionCurso;
    END IF;
    IF (validar_char(Seccion) = 0) THEN
        SELECT 'Error: La sección debe ser un caracter' as ERROR;
        LEAVE CrearSeccionCurso;
    END IF;

    INSERT INTO Seccion (CodigoCurso, Ciclo, CodigoCat, Seccion)
    VALUES (CodigoCurso, Ciclo, CodigoCat, Seccion);
    SELECT 'Éxito: Se ha creado la sección' as MENSAJE;
END $$
...
AgregarHorario:BEGIN
    ...
    SELECT 'Éxito: Se ha Creado Horario' as MENSAJE;
END $$
...
AsignarCurso:BEGIN
    IF (EVSeccion(Carnet, IdSeccion) = 1) THEN
        SELECT 'Error: Estudiante se encuentra ya asignado al curso en otra sección' as ERROR;
        LEAVE AsignarCurso;
    END IF;
    
    IF (AsignacionDuplicada(Carnet, IdSeccion) = 1) THEN
        SELECT 'Error: Ya esta asignado' as ERROR;
        LEAVE AsignarCurso;
    END IF;
    ...
    SELECT 'Éxito: Asignación realizada' as MENSAJE;
END $$
...
IngresarNota:BEGIN
    ...
    IF (ExisteEstudiante(Carnet) = 0) THEN
        SELECT 'Error: No existe el carnet de estudiante' as ERROR;
        LEAVE IngresarNota;
    END IF;
    IF (validar_enteroPositivo(nota_zona) = 0) THEN
        SELECT 'Error: La zona debe ser un número positivo' as ERROR;
        LEAVE IngresarNota;
    END IF;
    IF (validar_enteroPositivo(examen_final) = 0) THEN
        SELECT 'Error: La nota de examen debe ser un número positivo' as ERROR;
        LEAVE IngresarNota;
    END IF;
    -- obterner los creditos del curso, estan en pensum
    -- Insert
    INSERT INTO Notas (IdSeccion, Carnet, nota_zona, examen_final)
	VALUES (IdSeccion, Carnet, nota_zona, examen_final);
    SELECT 'Éxito: Nota ingresada correctamente' as MENSAJE;
END $$
...
DesasignarCurso:BEGIN
    ...
    END IF;
    IF (ExisteEstudiante(Carnet) = 0) THEN
        SELECT 'Error: No existe el carnet' as ERROR;
        LEAVE DesasignarCurso;
    END IF;
    IF (AsignacionDuplicada(Carnet, IdSeccion) = 0) THEN
        SELECT 'Error: El estudiante no esta asignado' as ERROR;
        LEAVE DesasignarCurso;
    END IF;
    -- Elimina el estudiante de Asignacion
    DELETE FROM Asignacion a WHERE a.IdSeccion = IdSeccion AND a.Carnet = Carnet;
    -- Insert
    INSERT INTO Desasignacion (IdSeccion, Carnet)
	VALUES (IdSeccion, Carnet); 
    SELECT 'Éxito: Desasignación correcta' as MENSAJE;
END $$
...
DECLARE ConsultarEstudiante_existente INT;
	IF (ExisteEstudiante(Carnet) = 0) THEN
        SELECT 'Error: El estudiante no existe' as ERROR;
...
ConsultarDocentee:BEGIN
	...
	IF (ExisteCatPorCodigo(CodigoCat) = 0) THEN
		SELECT 'Error: El catedratico no existe' as ERROR;
...
ConsultarHorario:BEGIN
	...
	IF (ExisteEstudiante(Carnet) = 0) THEN
        SELECT 'Error: No existe estudiante' as ERROR;
        LEAVE ConsultarHorario;
    END IF;
    IF (Año != 2024) THEN
        SELECT 'Error: No existe horario para esta fecha' as ERROR;
        LEAVE ConsultarHorario;
    END IF;
...
-- --------------------------------------------------------
DROP PROCEDURE IF EXISTS ConsultarAprobaciones;
DELIMITER $$
...
ConsultarAprobaciones:BEGIN
	...
	END IF;
	-- consulta
	SELECT s.CodigoCurso AS Curso, e.Carnet, CONCAT(e.Nombre, ' ', e.Apellido) AS NombreCompleto,
	CASE
		WHEN n.nota_zona + n.examen_final >= 61 THEN 'Aprobado'
		ELSE 'Reprobado'
	END AS Estado
    FROM Notas n
        JOIN Seccion s ON n.IdSeccion = s.IdSeccion
        JOIN Estudiante e ON n.Carnet = e.Carnet
	WHERE
        s.CodigoCurso = CodigoCurso;
END $$
DELIMITER ;
...
-- --------------------------------------------------------
--                CONSULTAR CURSOSAASIGNAR
-- --------------------------------------------------------
DROP PROCEDURE IF EXISTS ConsultarCursosAAsignar;
DELIMITER $$
CREATE PROCEDURE ConsultarCursosAAsignar(
    IN carnet INT
)
ConsultarCursosAAsignar:BEGIN
	IF (ExisteEstudiante(carnet) = 0) THEN
        SELECT 'Error: No existe el carnet de estudiante' as ERROR;
        LEAVE ConsultarCursosAAsignar;
    END IF;
	-- consulta
	SELECT DISTINCT C.CodigoCurso, C.Nombre AS 'Nombre del Curso',
    P.creditos_otorga AS 'Créditos que Otorga',
    PR.prerrequisito AS 'Código Prerrequisito Aprobado',
    CP.Nombre AS 'Nombre Prerrequisito'
	FROM Asignacion A
    JOIN Notas N ON A.IdSeccion = N.IdSeccion
    JOIN Curso C ON A.IdSeccion = C.CodigoCurso
    JOIN Pensum P ON C.CodigoCurso = P.CodigoCurso
    JOIN Prerrequisito PR ON C.CodigoCurso = PR.Curso
    JOIN Curso CP ON PR.prerrequisito = CP.CodigoCurso
...
--                INSERTAR COLUMNA
-- --------------------------------------------------------
DROP PROCEDURE IF EXISTS InsertarColumna;
DELIMITER $$
CREATE PROCEDURE InsertarColumna()
InsertarColumna:BEGIN
	-- Crear tabla auxiliar
    CREATE TABLE IF NOT EXISTS SalariosEnLetras (
    SalarioNumerico INT PRIMARY KEY,
    SalarioLetras VARCHAR(255) NOT NULL
);
	INSERT INTO SalariosEnLetras (SalarioNumerico, SalarioLetras) VALUES
	(1000, 'Mil'),
	(2000, 'Dos mil'),
	(3000, 'Tres mil');
    -- Agregar la columna SalarioEnLetras a la tabla Catedratico si no existe
    ALTER TABLE Catedratico ADD COLUMN SalarioEnLetras VARCHAR(255);

    -- Actualizar la columna SalarioEnLetras con los valores correspondientes en letras
    UPDATE Catedratico C
    JOIN SalariosEnLetras S ON C.salario = S.SalarioNumerico
    SET C.SalarioEnLetras = S.SalarioLetras;

    SELECT C.CodigoCat, CONCAT(C.Nombre, ' ', C.Apellido) AS NombreCompleto, C.salario, C.SalarioEnLetras
    FROM Catedratico C;
END $$
DELIMITER ;
